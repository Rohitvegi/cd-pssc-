...
class SE3Layer(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, pos):
        ...
        h_agg = torch.zeros_like(h_nodes)
...
class MultiScaleSE3Simple(nn.Module):
    ...
    def forward(self, graph):
        ...
        """
        x, edge_index, edge_attr, pos = graph.x, graph.edge_index, graph.edge_attr, graph.pos
        batch = graph.batch if hasattr(graph, 'batch') else None
        
        # If batch is None, create a batch tensor with all zeros
        if batch is None:
